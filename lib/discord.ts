...
export function headline(
  ...
  return {
    type: 9,
    components: [say([heading, ...lines].join("\n"))],
    accessory,
  };
}
...
export function embed(
  ...
  const container: Embed = {
    type: 17,
    components: pieces.filter(Boolean) as Piece[],
  };
...
function count(pieces: Piece[]): number {
  ...
  for (const piece of pieces) {
    total += 1;
    if (piece.type === 9)
      total += piece.components.length + (piece.accessory ? 1 : 0);
...
export function serialise(container: Embed): string | null {
  ...
  if (pieces > PIECES) {
    console.warn(
      `discord embed: ${pieces} components, over the ${PIECES} Discord renders`,
    );
    return null;
  }
  const json = JSON.stringify({ component: container }).replace(
    /</g,
    "\\u003c",
  );
  ...
  if (bytes > LIMIT) {
    console.warn(
      `discord embed: ${bytes} bytes, over the ${LIMIT} Discord reads`,
    );
    return null;
  }
  return json;
